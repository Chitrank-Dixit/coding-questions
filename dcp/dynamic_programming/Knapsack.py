...
def knapsack_memo(W, i, weights, values, dp):
    ...
    if weights[i] <= W:
        res = max(
            values[i]
            + knapsack_memo(W - weights[i], i - 1, weights, values, dp),
...
def knapsack_DP(W, weights, values):
    ...
    for i in range(1, len(weights) + 1):
        for w in range(0, W + 1):
            if weights[i - 1] <= w:
                dp[w][i] = max(
                    dp[w][i - 1], dp[w - weights[i - 1]][i - 1] + values[i - 1]
                )
            else:
                dp[w][i] = dp[w][i - 1]
...
def knapsack_DP_reconstruct(W, weights, values):
    ...
    n = len(weights)
    decisions = [
        [False for i in range(0, len(weights) + 1)] for j in range(0, W + 1)
    ]
    dp[W][0] = 0
